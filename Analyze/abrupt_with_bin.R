...
subj1 <- read.table('/Users/newxjy/Dropbox/VICE/JT/DEUBAL/Analyze/abrupt_subj1.csv', header=TRUE, sep=",")
subj2 <- read.table('/Users/newxjy/Dropbox/VICE/JT/DEUBAL/Analyze/abrupt_subj2.csv', header=TRUE, sep=",")
subj1 <- subj1[!(subj1$SN==1 & subj1$TN > 640 & subj1$TN <= 960),]
pilot_data <- rbind(subj1, subj2)
pilot_data$SN[641:1280] <- 2
...
discrim_trial <- discrim_trial[!(abs(discrim_trial$hand) < 20) & !(abs(discrim_trial$hand) > 40),] 
...
discrim_trial$match[discrim_trial$discrim_tar- discrim_trial$ti == 30] <- "-1"
discrim_trial$match[discrim_trial$discrim_tar- discrim_trial$ti == -60] <- "2"
discrim_trial$match[discrim_trial$discrim_tar- discrim_trial$ti == 60] <- "-2"
...
discrim_HTMT$bin <- floor(((discrim_HTMT$discrim_onset - discrim_HTMT$RT)*1000)/100)

main_count <- data_summary_count(discrim_HTMT, varname = c("Accuracy"), groupnames = c('bin', 'match', 'SN') )
main_sum   <- data_summary(discrim_HTMT, varname = c("Accuracy"), groupnames = c('bin', 'match', 'SN') )
main_plot  <- ggplot(main_sum[main_sum$match %in% c('MT', 'HT', "-1") & main_count$Accuracy > 5, ], aes(x = bin, y  = Accuracy, group = match, color = match)) + 
  geom_point(size = 3) + 
  geom_line() + 
  labs(color = "Location") +
  xlab("Movement Bin") + 
...
print(main_plot)
meanMT1 <- mean(discrim_trial$MT[discrim_trial$SN == 1]) #0.05740702
meanMT2 <- mean(discrim_trial$MT[discrim_trial$SN == 2]) #0.1209567
meanRT1 <- mean(discrim_trial$RT[discrim_trial$SN == 1]) #0.4115681
meanRT2 <- mean(discrim_trial$RT[discrim_trial$SN == 2]) #0.3269274
