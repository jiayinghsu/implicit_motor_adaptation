...
if InExpSystem
    ...
    screenNumber = max(screens);
    dir='/Users/jonathantsay/Dropbox/VICE/JT/DEUBAL/';
    mm2pixel = 3.00;
...
switch exp_type
    case 'covert'
        name_file = 'D_3P_COV';
        tgt_file = dlmread(strcat(name_file, subject_num,'.tgt'), '\t', 1, 0); % start reading in from 2nd row (1), 1st column (0)
    case 'veridical'
        name_file = 'D_3P_VERD';
        tgt_file = dlmread(strcat(name_file, subject_num,'.tgt'), '\t', 1, 0); % start reading in from 2nd row (1), 1st column (0)
    case 'gradual'
        switch rot_direction
            case 'clockwise'
                name_file = 'D_3P_GRAD_C';
                tgt_file = dlmread(strcat(name_file, subject_num,'.tgt'), '\t', 1, 0); % start reading in from 2nd row (1), 1st column (0)
            case 'counterclockwise'
                name_file = 'D_3P_GRAD_A';
                tgt_file = dlmread(strcat(name_file, subject_num,'.tgt'), '\t', 1, 0); % start reading in from 2nd row (1), 1st column (0)
        end
    ...
    case 'abrupt'
        switch rot_direction
            case 'clockwise'
                name_file = 'D_3P_ABRUPT_C';
                tgt_file = dlmread(strcat(name_file, subject_num,'.tgt'), '\t', 1, 0); % start reading in from 2nd row (1), 1st column (0)
            case 'counterclockwise'
                name_file = 'D_3P_ABRUPT_A';
                tgt_file = dlmread(strcat(name_file, subject_num,'.tgt'), '\t', 1, 0); % start reading in from 2nd row (1), 1st column (0)
        end
end
...
clamped_feedback = tgt_file(:,11); % 0= contingent- the value in 'rotation' is standard rotation;
...
while trial <= maxtrialnum   %
    ...
    if calibrate_eye(trial) == 10 % calibrate eye tracker
