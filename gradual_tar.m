...
exp_param = struct();
exp_param.sub = 4; 
exp_param.type = 'gradual';
exp_param.ver = 1; % version 1 is gradual, 2 is abrupt
...
nC_gradual = 15; % Abrupt rotation???? 15 for gradual??

nC_Discrim = length(discrim_trials) / exp_param.num_target; % Maybe this needs to be changed??

nT_total = (nC_FB + nC_nFB + nC_gradual + nC_Discrim) * exp_param.num_target; % Total number of trials
...
Trial.gradual(1) = Trial.nFB(2) + 1;
Trial.gradual(2) = Trial.nFB(2) + nC_gradual * exp_param.num_target; % block 3

Trial.Discrim(1) = Trial.gradual(2) + 1;
Trial.Discrim(2) = Trial.gradual(2) + nC_Discrim * exp_param.num_target; % block 4
...
header = {'trial_num','tgt_dist', 'discrim', 'discrim_onset', 'tgt_location',... 
    ...
    'endpoint_fb', 'between_blocks', 'raw_events', 'event_code'};
...
T.tgt_location(Trial.gradual(1):Trial.gradual(2)) = discrim_trials(Trial.gradual(1):Trial.gradual(2), 2);
...
% Is it a clamp trial: we are doing gradual so yes ??
...
switch exp_param.type
    ...
    case 'gradual'
        ...
        increment = exp_param.rot_size / ((Trial.gradual(2)-Trial.gradual(1) + 1));
        tot_increments = 0:increment:exp_param.rot_size;
        
        for m = 1:((Trial.gradual(2)-Trial.gradual(1) + 1))
            
            T.rotation(Trial.gradual(1) - 1 + m) = tot_increments(m);
        end
        
        T.rotation(Trial.gradual(2):Trial.Discrim(2)) = exp_param.rot_size;
        
    case 'abrupt'
        T.rotation(Trial.gradual(1):Trial.Discrim(2)) = exp_param.rot_size;
end
...
T.between_blocks(Trial.gradual(2)) = 3;
